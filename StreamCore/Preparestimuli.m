...
for(stim = 1:length(stimuli))
    ...
    switch(stimtype)
        case 1     %this is a texture stimulus, which means we load it from an image file
            ...
            if(stim > 1 && shortcut)
                Stimuli_set.stimsize(stim) = Stimuli_set.stimsize(1);
                Stimuli_set.rotate(stim) = stimstruct.rotate(stim);
                Stimuli_set.leftjustify(stim) = Stimuli_set.leftjustify(1);
                Stimuli_set.stimnames{stim} = Stimuli_set.stimnames{1}; % list of stimuli
                Stimuli_set.stimuli(stim) = Stimuli_set.stimuli(1);
               % Stimuli_set.color(stim) = [];
                Stimuli_set.xdim(stim) = Stimuli_set.xdim(1);
                Stimuli_set.ydim(stim) = Stimuli_set.ydim(1);
                Stimuli_set.bottomjustify(stim) = Stimuli_set.bottomjustify(1);
                Stimuli_set.clippingboundary(stim) = Stimuli_set.clippingboundary(1);
                Stimuli_set.transparencythreshold(stim) = Stimuli_set.transparencythreshold(1);
                Stimuli_set.savestim(stim) = Stimuli_set.savestim(1);
